...
// Load questions from JSON files (module 4 + module 5)
async function loadQuestions() {
    try {
        ...
        if (!r4.ok) throw new Error(`HTTP ${r4.status} loading questions.json`);
        if (!r5.ok) throw new Error(`HTTP ${r5.status} loading questions5.json`);
        const [d4, d5] = await Promise.all([r4.json(), r5.json()]);
        allQuestions = [...d4.questions, ...d5.questions];
...
function getModuleQuestions() {
    return allQuestions.filter(q =>
        selectedModule === 5 ? q.module === 5 : (!q.module || q.module === 4)
    );
}
