...
function quizTemplate(moduleNum, moduleTitle, questionsJson) {
  const pad = String(moduleNum).padStart(2, '0');
  const safeJson = questionsJson.replace(/</g, '\\u003c').replace(/>/g, '\\u003e');
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Module ${moduleNum} Quiz — Discrete Math for CS</title>
  <link rel="stylesheet" href="style.css">
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&family=Fira+Code:wght@400;500&display=swap" rel="stylesheet">
</head>
<body data-page="quiz-${pad}">
  <button id="nav-toggle" aria-label="Toggle navigation">☰</button>
  <nav id="sidebar"></nav>
  <main id="main">
    <div class="page-content">
      <h1>Module ${moduleNum}: Practice Quiz</h1>
      <p class="quiz-subtitle">${moduleTitle} — Test your understanding with ${JSON.parse(questionsJson).length} multiple-choice questions.</p>
      <div id="quiz-container" data-questions='${safeJson}'></div>
    </div>
  </main>
  <script src="https://cdn.jsdelivr.net/npm/katex@0.16.9/dist/katex.min.js"></script>
  <script src="https://cdn.jsdelivr.net/npm/katex@0.16.9/dist/contrib/auto-render.min.js"></script>
...
function build() {
  ...
  // Quiz pages
  const mcqData = require('./mcq-data.js');
  const quizTitles = [
    'Integers, Definitions & Proofs',
    'Remainders, Modular Arithmetic & Primes',
    'Logic, Quantifiers & Program Reasoning',
    'Sets, Set Operations & Data Collections',
    'Functions, Relations & Mappings',
    'Induction, Recursion & Recursive Definitions',
    'Counting, Growth & Finite Reasoning',
    'Algorithms, Loop Invariants & Correctness',
    'Trees, Complexity & Verification',
  ];
  for (let m = 1; m <= 9; m++) {
    const pad = String(m).padStart(2, '0');
    const questions = mcqData[m - 1] || [];
    if (questions.length > 0) {
      const quizHtml = quizTemplate(m, quizTitles[m - 1], JSON.stringify(questions));
      fs.writeFileSync(path.join(SITE_DIR, `quiz-${pad}.html`), quizHtml);
      console.log(`  ✓ quiz-${pad}.html`);
    }
  }
