...
INPUT_CSV  = sys.argv[1] if len(sys.argv) > 1 else os.path.join(DATA_DIR, "forsale_urls.csv")
OUT_PROPS  = os.path.join(DATA_DIR, "properties.csv")
OUT_ASSESS = os.path.join(DATA_DIR, "assessments.csv")
OUT_HIST   = os.path.join(DATA_DIR, "mls_history.csv")
...
def label_val(soup, label_text, skip_template=False):
    # Primary: <div class="cutsheet-detail-item">Label:<span>Value</span></div>
    for div in soup.find_all(class_="cutsheet-detail-item"):
        span = div.find("span")
        if not span:
            continue
        raw = div.get_text(" ", strip=True)
        span_text = span.get_text(" ", strip=True)
        label_part = raw.replace(span_text, "").strip().rstrip(":").strip()
        if label_text.lower() in label_part.lower():
            if skip_template and "{{" in span_text:
                continue
            val = span_text.strip()
            return val if val and val.upper() != "N/A" else None
    # Fallback: separate label/value sibling structure (older pages)
...
def scrape(cutsheet_url, session):
    ...
    try:
        ...
        result["heating_cooling"]     = label_val(soup, "Heating/Cooling") or label_val(soup, "Heating")
        ...
        result["water_access_view"]   = label_val(soup, "Water Access/View") or label_val(soup, "Water Access")
        ...
        result["appliances"]          = label_val(soup, "Appliances Incl.") or label_val(soup, "Appliances")
        ...
        result["betterment_charges"]  = label_val(soup, "Betterment Charges") or label_val(soup, "Betterment")
        _bank = label_val(soup, "Bank Owned")
        result["bank_owned"]          = 1 if str(_bank).lower() == "yes" else (0 if _bank is not None else None)
        # also grab bathrooms_fh with correct label
        result["bathrooms_fh"]        = label_val(soup, "Bathrooms (F/H)") or label_val(soup, "Bathrooms")
